...
Controller lua_hiddbg_AttachController(u32 bodyColor, u32 buttonsColor, u32 gripLColor, u32 gripRColor)
{
    ...
    if (R_FAILED(rc))
    {
        throw std::format("Error attaching controller: %#x", rc);
    }
    ...
    if (R_FAILED(rc))
    {
        throw std::format("Error setting controller state: %#x", rc);
    }
...
void lua_hiddbg_DetachController(Controller *controller)
{
    ...
    if (R_FAILED(rc))
    {
        throw std::format("Error disconnecting controller: %#x", rc);
    }
...
//     if (R_FAILED(rc))
//     {
//         throw std::format("Error checking if controller attached: %#x", rc);
//     }
...
void lua_hiddbg_SetButtons(Controller *controller, u64 buttons)
{
    ...
    if (R_FAILED(rc))
    {
        throw std::format("Error setting button state: %#x", rc);
    }
...
void lua_hiddbg_SetJoystick(Controller *controller, int stickIndex, s32 x, s32 y)
{
    ...
    if (R_FAILED(rc))
    {
        throw std::format("Error setting joystick state: %#x", rc);
    }
